...
For development or dummy environments for a quick demo, one would insert the contents of `secrets.yaml` which can be conveniently generated by `secrets-setup.sh` and/or adjusted manually. In addition to that, include the contents of `regcred.yaml` if you need the image pull secret to be created for you without using external secrets.

### Production-ready approach

Instead of storing secrets directly in the manifest, leave `valuesObject` empty and source secrets from a secure secret management solution such as Vault. To do so, toggle `general.install.externalSecret.*`. Describing Vault setup itself is out of the scope of this document, but [Vault integration](VAULT.md) is documented in detail.
...
- `general.license.ldapKey`
- `general.license.isvdiKey`
